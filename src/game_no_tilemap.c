...
player_draw(game_state *GameState, back_buffer *BackBuffer, player *Player)
{
	...
	v2f MinPos = point_map_to_screen(GameState, BackBuffer, Player->Pos);
...
{
	v2f Min;
	Min.x = (f32)TileMapPos.TilePos.x * GameState->tile_side_in_meters;
	Min.y = (f32)TileMapPos.TilePos.y * GameState->tile_side_in_meters;

	v2f Max;
	Max.x = Min.x + GameState->tile_side_in_meters;
	Max.y = Min.y + GameState->tile_side_in_meters;
...
update_and_render(game_memory *GameMemory, back_buffer *BackBuffer, sound_buffer *SoundBuffer, game_input *GameInput)
{
	...
	if (GameInput->Controller.Space.ended_down) {
		if (!Player->is_shooting) {
			if (GameState->projectile_count < ARRAY_COUNT(GameState->Projectiles)) {
				for (u32 projectile_index = 0; projectile_index < ARRAY_COUNT(GameState->Projectiles); projectile_index++) {
					projectile *Projectile = &GameState->Projectiles[GameState->projectile_count];
	...
	for (u32 projectile_index = 0; projectile_index < ARRAY_COUNT(GameState->Projectiles); projectile_index++) {
		...
		if (Projectile->active) {
			...
			Projectile->time_between_next_trail -= 4.0f * dt_for_frame;
			if (Projectile->time_between_next_trail <= 0.0f) {
				...
				// Visually this makes the projectile look like a beam of energy
				...
				// until the time left is <= 0 which is fully transparent.
	...
	v2f PlayerNewPos = v2f_add(Player->Pos, PosOffset);
	...
	RightOffset = v2f_add(DownOffset, RightOffset);
	v2f PlayerNewVertexRight = v2f_add(PlayerNewPos, RightOffset);

	v2f LeftOffset = v2f_scale(-1.0f * Player->base_half_width, PlayerNewRight);
	LeftOffset = v2f_add(DownOffset, LeftOffset);
	v2f PlayerNewVertexLeft = v2f_add(PlayerNewPos, LeftOffset);

	v2f TopOffset = v2f_scale(Player->height / 2.0f, PlayerNewDirection);
	v2f PlayerNewVertexTop = v2f_add(PlayerNewPos, TopOffset);

	v2f BezierTopOffset = v2f_scale(-0.5f, TopOffset);
	v2f PlayerNewVertexBezierTop = v2f_add(PlayerNewPos, BezierTopOffset);
